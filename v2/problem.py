...
class DEAData:
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(self, "input_names", _validate_names("input", self.input_names, inputs.shape[1]))
        object.__setattr__(
            self,
            "good_output_names",
            _validate_names("good_output", self.good_output_names, good_outputs.shape[1]),
        )
        bad_count = 0 if bad_outputs is None else bad_outputs.shape[1]
        object.__setattr__(
            self,
            "bad_output_names",
            _validate_names("bad_output", self.bad_output_names, bad_count),
        )
